...
class LocationManager {
    private val locationConfigs = LocationConfigLoader.load()

    fun getMobsForLocation(location: Location): List<Mob> {
        return locationConfigs[location]?.mobs?.map { it.copy() } ?: emptyList()
    }
}

// model/LocationConfig.kt
data class LocationConfig(
    val name: String,
    val mobs: List<MobTemplate>
)
...
data class MobTemplate(
    ...
    val minLevel: Int,
    val maxLevel: Int,
    val positionRange: Pair<FloatRange, FloatRange>
)